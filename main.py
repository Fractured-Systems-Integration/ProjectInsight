...
# Try ttkbootstrap for modern themes
try:
...
class App:
    def __init__(self, root, style):
        self.root = root
        self.style = style  # <- keep a reference; don't assign to root.style
        self.root.title(f"{APP_NAME} v{VERSION}")
        self.root.geometry("1100x700")

        # If you want to force a starting theme:
        try:
            if _HAS_TTKB:
        ...
        try:
            ...
            self._logo_photo = logo_photo  # keep reference
        except Exception:
    ...
    def _build_menu(self):
        ...
        help_menu.add_command(
            label="About",
            command=lambda: messagebox.showinfo("About", f"{APP_NAME} v{VERSION}\n© 2025 Fractured Systems Integration")
        )
...
def main():
    if _HAS_TTKB:
        ...
        style = root.style                      # get the existing Style (read-only attribute)
    else:
        root = tk.Tk()
        style = ttk.Style()
